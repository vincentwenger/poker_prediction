...
- Make all our models play against each other’s using a poker game engine. Then create some tournaments to figure out which models beat the others in real poker games. We could explore if the winners will be the same ones that we found have the best performance metrics in this notebook. I did this in the 5th notebook about "Game testing". The results from this notebook were that:
